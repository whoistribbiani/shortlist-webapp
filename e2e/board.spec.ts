...
test("selects a player with autocomplete flow and enriches the slot", async ({ page }) => {
  ...
  await expect(board).toHaveAttribute("data-zoom", "75");
  await expect(page.getByTestId("zoom-reset")).toHaveText("75%");
  ...
  const zoom80SlotWidth = await firstCard.evaluate((element) => element.getBoundingClientRect().width);
  expect(zoom80SlotWidth).toBeGreaterThan(defaultSlotWidth);
  await page.getByTestId("zoom-out").click();
  await expect(board).toHaveAttribute("data-zoom", "75");
  await page.getByTestId("zoom-reset").click();
  await expect(board).toHaveAttribute("data-zoom", "100");
  const zoom100LabelHeight = await firstPlayerLabel.evaluate((element) => element.getBoundingClientRect().height);
  expect(zoom100LabelHeight).toBeGreaterThan(defaultLabelHeight);
  await page.getByTestId("zoom-in").click();
  ...
  expect(zoom110SlotWidth).toBeGreaterThan(zoom80SlotWidth);
  await page.getByTestId("zoom-reset").click();
  await expect(board).toHaveAttribute("data-zoom", "100");
